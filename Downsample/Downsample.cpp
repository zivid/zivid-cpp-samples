...
Zivid::PointCloud downsample(const Zivid::PointCloud& pointCloud, int downsamplingFactor)
{
	...
	*/
	...
	{
		throw std::invalid_argument("Downsampling factor (" + std::to_string(downsamplingFactor) + ") has to be divisible by the width (" + std::to_string(pointCloud.width()) + ") and height (" + std::to_string(pointCloud.height()) + ") of the input point cloud, e.g. 2, 3, 4, 5, 6.");
	}
