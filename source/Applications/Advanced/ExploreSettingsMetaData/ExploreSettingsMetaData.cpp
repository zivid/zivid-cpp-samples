...
void printValidRange(const Zivid::CameraInfo &cameraInfo) {
  const auto range =
      Zivid::Experimental::SettingsInfo::validRange<Node>(cameraInfo);

  std::cout << "  ValidRange: [" << Node{range.min()} << " , "
            << Node{range.max()} << "]\n";
}
...
void printValidValues(const Zivid::CameraInfo &cameraInfo) {
  const auto validValues =
      Zivid::Experimental::SettingsInfo::validValues<Node>(cameraInfo);
...
void inspectLeaf(const Node &node, const Zivid::CameraInfo &cameraInfo) {
  ...
  if constexpr (DecayedNode::nodeType ==
    ...
    if constexpr (Zivid::DataModel::IsOptional<DecayedNode>::value) {
      ...
      if (node.hasValue()) {
        ...
        const auto defaultValue =
            Zivid::Experimental::SettingsInfo::defaultValue<DecayedNode>(
                cameraInfo);
    ...
    constexpr bool hasRange =
        Zivid::DataModel::HasValidRange<DecayedNode>::value;
    constexpr bool hasValues =
        Zivid::DataModel::HasValidValues<DecayedNode>::value;
...
void traverseLeavesIntoNestedDataModelsAndLists(
    Node &node, const LeafVisitor &leafVisitor) {
  using DecayedNode = std::decay_t<Node>;

  if constexpr (DecayedNode::nodeType == Zivid::DataModel::NodeType::group ||
                DecayedNode::nodeType ==
                    Zivid::DataModel::NodeType::leafDataModelList) {
    std::forward<Node>(node).forEach([&](auto &&childNode) {
      traverseLeavesIntoNestedDataModelsAndLists(
          std::forward<decltype(childNode)>(childNode), leafVisitor);
    });
  } else if constexpr (DecayedNode::nodeType ==
                           Zivid::DataModel::NodeType::leafValue &&
...
void traverseAndPrint(const SettingsType &settings,
                      const Zivid::CameraInfo &cameraInfo) {
  traverseLeavesIntoNestedDataModelsAndLists(
      settings, [&](const auto &node) { inspectLeaf(node, cameraInfo); });
}
} // namespace
...
int main() {
  try {
    ...
    auto defaultSettings2DWithAcquisition =
        Zivid::Experimental::SettingsInfo::defaultValue<Zivid::Settings2D>(
            cameraInfo)
            .copyWith(Zivid::Settings2D::Acquisitions{
                Zivid::Experimental::SettingsInfo::defaultValue<
                    Zivid::Settings2D::Acquisition>(cameraInfo)});
