...
namespace
{
        ...
        CSVLogger &csvLogger)
    {
        ...
        for(const auto &preset : presets)
        {
            ...
            const std::array<std::pair<std::string, std::vector<Duration>>, 3> measurements{
                std::make_pair(" acquisition", acquisitionDurations),
                std::make_pair(" processing", processDurations),
                std::make_pair(" total", totalDurations)
            };
            for(const auto &[label, durations] : measurements)
            {
                const auto median = computeMedianDuration(durations);
                const auto mean = computeAverageDuration(durations);
                printFormatted({ "  " + preset.name + label, formatDuration(median), formatDuration(mean) });
                csvLogger.logBenchmarkResult(
                    "capture_preset",
                    preset.name + label,
                    1,
                    ...
                    cameraModel,
                    serialNumber,
                    computeStandardDeviationMs(durations));
            }
        }
    }
